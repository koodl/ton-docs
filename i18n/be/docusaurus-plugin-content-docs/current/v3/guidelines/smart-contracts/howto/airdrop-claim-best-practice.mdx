...
In that case each distributor would process _merkle root_ which consists entirely from users originating from it's shard.
...
One requirement is that _distributor_ contract has to have updatable _merkle root_
...
- For each user find such index, so it's _double spend_ contract `(distributor, index)` ends up in same shard as the user address.
- Generate _merkle roots_ with indexes from step above
- Update _distributors_ with according _merkle roots_

Should be good to go now!

### V3

- User deploys _double spend_ contract in the same shard using index tuning
...
- Same as V3 but 16 wallets now, each in same shard as it's _distributor_.
- Going to have to make _fee wallet_ address updatable
