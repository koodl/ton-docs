...
1. _Block size estimation_ - not an actual block size, but an estimation calculated during collation.
2. _Gas consumption_ - total gas consumed in all transactions (excluding ticktock and mint/recover special transactions).
3. _Lt delta_ - difference between start and end lt of the block.
...
1. _Block size_: `128/256/512 KiB`.
2. _Gas consumption_: `2M/10M/20M` in basechain, `200K/1M/2.5M` in masterchain.
