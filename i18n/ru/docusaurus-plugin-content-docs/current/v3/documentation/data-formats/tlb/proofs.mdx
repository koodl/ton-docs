...
# Проверка подтверждений (низкий уровень)

## Общий обзор

:::caution
Этот раздел описывает инструкции и руководства по взаимодействию с TON на низком уровне.
...
Эта статья описывает дополнительные примеры проверки доказательств с Liteservers.

Важно проверять любые данные, полученные с узла для бесконтрольного взаимодействия с блокчейном.
...
И мы просим заголовок Liteserver для этого блока. Liteserver [response](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/tl/generate/scheme/lite_api.tl#L35)
содержит `header_proof` boc.
...
После десериализации бока мы получили Гены:
...
что мы должны десериализовать в соответствии с Блоком [схемой Tlb](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L442):
...
Now, we should check that `seqno` in deserialized block matches to block `seqno` we know, and then compute hash_1 of the only
Merkle Proof reference and compare it to block hash we know:

```python
утверждает h_proof.refs[0].get_hash(0) == block_id.root_hash
```

Теперь мы можем доверять всем остальным данным, эта ячейка содержит

_Проверка доказательств:_ [Python](https://github.com/yungwine/pytoniq-core/blob/873a96aa2256db33b8f35fbe2ab8fe8cf8ae49c7/pytoniq_core/proof/check_proof.py#L19), [Kotlin](https://github.com/andreypfau/ton-kotlin/blob/b1edc4b134e89ccf252149f27c85fd530377cebe/ton-kotlin-liteclient/src/commonMain/kotlin/CheckProofUtils.kt#L15), [C+](https://github.com/ton-blockchain/ton/blob/24dc184a2e67f9c47042b

## Полный блок

Для метода `liteserver.getBlock` проверка доказательств совпадает с описанным выше, однако она содержит полные Гены вместо ветвей для [Value Flow](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block. lb#L464), [Обновление состояния](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block.tlb#L412-L413) и [Экстра](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L452)

## Шард Блок

Доказательством является то, что шард на самом деле хранится в блоке masterchin, который мы предоставляем Liteserver. При вызове методов `liteServer.getShardInfo`, `liteServer.getAccountState` и `liteServer.runSmcMethod`.

Попросим Liteserver информацию об шарде для упомянутого выше блока masterchine:

```python
await client.raw_get_shard_info(master, wc=0)
```

Liteserver response содержит BlockIdExt блока шард:
...
И бокс `shard_descr`, который мы можем использовать в результате если доверяем Liteserver.
...
После десериализации осколков мы получили 2 корня:
...
Первым является блок Masterchain Merkle Pro, который мы должны проверить (используя `check_block_header`):
...
Обновление Merkle для [ShardState](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L412-L413) TLB схема, поэтому нам нужно помнить о новом хэше.

После того, как мы убедились в том, что единственный Merkle Proof Cell ссылается на хэш блока, который мы знаем, и вспомнили новый хэш ShardState , мы проверяем вторую ячейку `shard proof`:
...
Как мы видим, единственный Merkle Proof имеет префикс `9023AFE2`, который [ShardStateUnsplit](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block. lb#L410)
префикс TLB, поэтому нам нужно сравнить эту ссылку Hash_1 с той, которую мы помнили в предыдущем шаге:

```python
"""
Здесь mc_block_cell — первый корень надежной системы, а mc_state_root — второй.
Функция check_block_header_proof возвращает новый хэш обновления ShardState Merkle.
"""

mc_state_hash = mc_state_root[0]. et_hash(0)
state_hash = check_block_header_proof(mc_block_cell[0], blk.root_hash, True)

если mc_state_hash != state_hash:
    raise ProofError('mc состояние хэшей не совпадает')
```

- _Почему?_ - потому что мы проверили защиту от блоков и это означает, что мы можем доверять другим данным ячеек. Так что теперь мы верим в новый хэш ShardState Merkle, и доверять второй информации о сетях нам нужно проверить, совпадают ли хэши.

Теперь давайте десериализовать вторую ячейку:
...
Поскольку мы доверяем этой ячейке мы можем доверять данные об фрагменте (`ShardStateUnsplit` -> `custom` -> `shard_hashes` -> `0 (shrdblk wc)` -> `leaf`).

## Состояние аккаунта

Давайте докажем состояние аккаунта `EQBvW8Z5huBkMJYdnfAEM5JqTNkuWX3diqYENkWsIL0XggGG` за тот же блок Masterchin, с которого мы начали в начале статьи.

Liteserver [response](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/tl/generate/scheme/lite_api. l#L37) содержит идентификатор блока masterchain _(должен быть таким же, как мы отправили на ls)_, блоки шарда id и «shard_proof», которые мы должны проверить, как описано выше, как описано выше,
«proof» и бок «state».
...
Когда мы проверили `Shard Proof`, нам нужно десериализовать и `proof` ячейки `state`. Во-первых, доказательная клетка должна иметь ровно 2 корня:
...
Первый корень является аргументом Merkle для блока шарда (мы уже доказали и доверяем ему хэш):
...
Как мы делали в процессе проверки `Shard Proof`, нам нужно использовать функцию `check_block_header`: проверьте, верна ли Блок ячейка и запомните новый хэш `StateUpdate`.

Затем мы десериализируем второй корень (давайте назовем его `state_cell`) и проверьм, соответствует ли его хэш хэш:

```python
proof_cells = Гена. rom_boc(proof)
если len(proof_cells) ! 2:
    сбор ProofError('ожидалось 2 корневых ячейки в доказательстве состояния учетной записи')

state_cell = proof_cell[1]

state_hash = check_block_header_proof(proof_cells[0][0], shrd_blk. oot_hash, True)

если state_cell[0].get_hash(0) != state_hash:
    поднимают ProofError('состояние хэшей не совпадает')
```

Теперь мы можем доверять `state_cell`, который выглядит следующим образом:
...
Опять же, единственная ссылка Merkle Proof имеет префикс `9023AFE2`, который [ShardStateUnsplit](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block. lb#L410)
TLB префикс, так что мы будем десериализовать его в соответствии с схемой TLB:
...
Здесь нам нужно поле `account`, которое имеет [ShardAccounts](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L261)
`ShardAccounts` — это HashmapAugE, где ключом является hash_part, значение имеет тип `ShardAccount`, а дополнительно имеет тип `DeepBalanceInfo`.

Parsing the address `EQBvW8Z5huBkMJYdnfAEM5JqTNkuWX3diqYENkWsIL0XggGG` we get the hash_part is equals to `50368879097771769677871174881221998657607998794347754829932074327482686052226`, so we need to
...
```python
{
    50368879097771769677871174881221998657607998794347754829932074327482686052226: {
        'account': None,
        'last_trans_hash': b'd\x9bF\xacr\xe6\xe4\xd4\xc1);\xd5\x8d\x9e\xd7\xa5I\x02\xbe\xef\xd9\x7f[\xffyw\xdd\x85\x99\x8b\x8b =',
        'last_trans_lt': 39330697000001,
        'клетка': <Cell 320[649B46AC72E6E4D4C1293B66D58D9ED7A54902BEEFD97F5BFF7977DD85998B3D000023C564393441] -> 1 refs>
    }
}
```

Нам нужно запомнить `last_trans_hash` и `last_trans_lt`, потому что мы можем использовать их для получения транзакций по счету, и давайте проверим всю информацию о ячейке:
...
Как мы видим, ячейка является обычной ячейкой с уровнем 1, который имеет только одну ссылку - выверенные данные учетной записи, так что давайте посчитаем хэш Hash_1 этой удаленной ветки -
это хэш учетной записи: `8282d13bf66b9ace1fbf5d3abd1c59cc46d61af1d47af1665d3013d8f9e47488`.

Теперь последним шагом является десериализация блока «state»:
...
Вычислить его хэш представления и убедиться, что он соответствует тому, что мы получили от Prun: `8282d13bf66b9ace1fbf5d3abd1c59cc46d61af1d47af1665d3013d8f9e47488`.

И десериализовать его согласно [Account](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L231-L233) TLB схема:
...
Теперь мы можем доверять этим данным учетной записи.

## Транзакции по счету

Для [liteServer.getTransactions](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/tl/generate/scheme/lite_api.tl#L71) запрос мы должны предоставить `lt` и `hash` транзакции для начала транзакции.
Если мы хотим получить последние транзакции по счету, мы можем получить их из `ShardAccount` (описано выше) и доверять этим `lt` и `hash`.

Когда мы получаем транзакции с Liteserver, мы получаем boc с количеством транзакций, которые мы спросили roots. Каждый корень — это ячейка, которую мы должны десериализовать согласно [Transaction](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block.tlb#L263-L269) TLB Схема.
Для первой транзакции мы должны проверить, что ее хэш совпадений с `last_trans_hash` мы получили из состояния аккаунта. Затем мы запомним значение поля `prev_trans_hash` и сравниваем его с хэшем второго корня и так далее.

## Транзакции блока

Давайте попросим Liteserver о транзакциях принадлежат блоку, с которого мы начали в начале статьи.
LiteServer [response](https://github.com/ton-blockchain/ton/blob/master/tl/generate/scheme/lite_api.tl#L46) содержит поле `ids` с транзакциями и `proof` boc. Сначала давайте десериализируем `proof`:
...
Теперь мы должны проверить знак заголовка блока (чтобы доверять этим данным ячейки) и десериализовать его в соответствии с схемой Блок TLB:
...
В этом случае мы должны помнить поле `block` -> `extra` -> `account_blocks`, который имеет тип [ShardAccountBlocks](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block. lb#L282),- HashmapAugE ключ адреса хэш_часть, значение имеет [AccountBlock](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block.tlb#L277-L280) тип и дополнительный тип `CurrencyCollection`:
...
Теперь давайте посмотрим `ids`:

```python
[
    {'mode': 39, 'account': '333333333333333333333333333333333333333333333333333333333333333333333333333333', 'lt': 39391488000001, 'hash': '5ef0532af460bcf3becf1a94597c1ec04879e0f26bf58269d319121376aad473'},
    {'mode': 39, 'account': '33333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333}, 'lt': 39391488000002, 'hash': 'b1e091fcb9df53917eaa0cae05041b3d0956242871e3ca8d6909d0aa31ff3604'},
    {'mode': 39, 'account': '555555555555555555555555555555555555555555555555555555555555555555555555555555555555', 'lt': 39391488000003, 'hash': '924b5992df95114196994a6d449d89e1c002cb96c14d11c4a667f843a3faf441'}
]
```

Для каждой транзакции здесь нам нужно найти её в `account_block` мы запомним и сравнить хэши с хэшами:

```python
block_trs: dict = acc_block.get(int(tr['account'], 16)).транзакции[0]
...
В этом примере было ненужно проверить поле `ids`, мы можем просто принимать транзакции из блока учетной записи.
...
Давайте попросим Liteserver на 1, 4, 5, 7, 8 и 15 [Параметры конфигурации](https://github.com/ton-blockchain/ton/blob/master/tl/generate/scheme/lite_api. l#L83) (для [liteServer.getConfigAll](https://github.com/ton-blockchain/ton/blob/master/tl/generate/scheme/lite_api.tl#L82), где вы получаете все параметры, проверка правильности совпадает).
...
Сначала давайте десериализовать ячейку `state_proof`:
...
Для этого мы должны проверить Block Header proof и запомнить `StateUpdate` новый хэш.

Теперь давайте десериализируем ячейку `config_proof`:
...
Нам нужно сравнить хеш_1 только ссылки Merkle Proof с хэшем из функции `check_block_header` выше, чтобы мы могли доверять этой ячейке:

```python
state_hash = check_block_header_proof(state_proof[0], block.root_hash, True)
если config_proof[0].get_hash(0) != state_hash:
    raise LiteClientError('хэши ошибки')
```

Теперь давайте десериализируем ячейку согласно схеме `ShardStateUnsplit`:
...
И возьмите `ShardStateUnsplit` -> `custom` -> `config` -> `config` , которая является Hashmap, где ключом является номер ConfigParam и значение является ячейка с значением параметра.

После десериализации всех доступных параметров:
